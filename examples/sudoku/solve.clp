...
(defrule eliminate-not-employed
   ...
   =>
   (retract ?f1 ?f2)
   ;;(printout t "RET PO " ?rr ?cr ?v " gr " ?gr " " ?id " ene " ?r crlf))
...
(defrule eliminate-employed
   ...
   =>
   ;;(printout t "RET PO " ?rr ?cr ?v " gr " ?gr " " ?id " ee " ?r crlf))
   (retract ?f1 ?f2))
...
(defrule naked-triples-row
   ...
   (printout t :IMPO-R " " ?id1 " " ?v " " ?p " Naked Triples" crlf)
...
(defrule naked-triples-column
   ...
   (printout t :IMPO-C " " ?id1 " " ?v " " ?p " Naked Triples" crlf)
   (assert (impossible (id ?id) (value ?v) (rank ?p) (reason "Naked Triples"))))
...
(defrule naked-triples-group
   ...
   (printout t :IMPO-G " " ?id1 " " ?v " " ?p " Naked Triples" crlf)
   (assert (impossible (id ?id) (value ?v) (rank ?p) (reason "Naked Triples"))))
...
(defrule continue-chain-row
   ...
   =>
   ;;(printout t "CR " ?sr ?sc ?sv " " ?r ?c2 ?g " " ?v2 " " ?id crlf) 
...
(defrule continue-chain-column
   ...
   =>
   ;;(printout t "CC " ?sr ?sc ?sv " " ?r2 ?c ?g " " ?v2 " " ?id crlf) 
   (assert (chain (start-row ?sr)
                  (start-column ?sc)
                  (start-value ?sv)
                  (row ?r2)
...
(defrule continue-chain-group
   ...
   =>
   ;;(printout t "CG " ?sr ?sc ?sv " " ?r ?c ?g " " ?v2 " " ?id2 crlf) 
   (assert (chain (start-row ?sr)
                  (start-column ?sc)
                  (start-value ?sv)
                  (row ?r)
...
(defrule forced-chain-convergence
   ...
   =>
   (printout t "FCC ID " ?id " VAL " ?v3 " RANK " ?p crlf)
...
(defrule forced-chain-XY
   ...
   (printout t "FCXY ID " ?id " VAL " ?v1 " RANK " ?p crlf)
